...
def trace(ray, objects, light, maxRecur):
    ...
    if intersect.d == -1:
        ...
        # print("color is set to ambientx: ", col.x)
        # print("color is set to ambienty: ", col.y)
        # print("color is set to ambientz: ", col.z)

    elif intersect.n.dot(light - intersect.p) < 0:
        col = intersect.obj.col * AMBIENT
        # print("Intersect col times ambient x: ", col.x)
        # print("Intersect col times ambient y: ", col.y)
        # print("Intersect col times ambient z: ", col.z)
        ...
        if testRay(lightRay, objects, intersect.obj).d == -1:
            ...
            col = intersect.obj.col * \
                max(intersect.n.normal().dot(
                    (light - intersect.p).normal()*lightIntensity), AMBIENT)
...
objs.append(Plane(Vector(0, 0, -12), Vector(0, 0, 1),
                  Vector(255, 255, 255)))  # normal, point, color

# experiment with a different (x,y,z) light position
lightSource = Vector(10, 0, 0)
